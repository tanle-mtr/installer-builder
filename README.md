...
- 🖱️ **选择主程序**：支持现成 `.exe`，或 **Python 入口脚本**（`.py/.pyw`，自动经 PyInstaller 打包为 exe 壳）；附加文件/文件夹随包安装
...
2. **选择主程序**：现成 `.exe`，或 Python 入口脚本（`.py/.pyw`，自动先打成 exe 壳）；依赖文件/文件夹通过"添加"加入附加列表（Python 模式将内嵌进主程序）
3. 勾选安装选项：桌面快捷方式 / 开始菜单快捷方式 / 开机自启
4. 可选：自定义图标（`.ico/.png/.jpg`，留空用默认图标）；选择输出目录
5. 点击 **生成安装包**，约 1~3 分钟得到：
